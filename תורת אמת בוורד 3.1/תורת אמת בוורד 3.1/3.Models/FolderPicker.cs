...
{
    public static class FilePickerX
    {
    ...
    public static class FolderPickerLauncher
    {
        public static string Pick_A_Folder(string folderPickerTitle)
        {
            using (var folderPicker = new FolderPicker())
            {
                folderPicker.Title = folderPickerTitle;
        ...
        public static string[] GetAllSelectedFolders(string title) 
        {
            FolderPicker folderPicker = new FolderPicker { Title = title, };
            folderPicker.Multiselect = true;

            if (folderPicker.ShowDialog(IntPtr.Zero) == true)
            {
                return folderPicker.ResultPaths.ToArray();
            }
            return new string[0];
        }
        ...
        {
            string folder = FolderPickerLauncher.Pick_A_Folder("בחר תיקייה");
            ...
            {
                MessageBox.Show($"Error getting files: {ex.Message}");
                return new string[0];
            }
        }

        public static string[] GetFoldersArray()
        {
            string folder = FolderPickerLauncher.Pick_A_Folder("בחר תיקייה");
            if (folder == null) { return new string[0]; }
            try
