...
if (IsExpanded) { 
        ...
        public static async void SetScrollPosition(WebView2 webView, double scrollPosition, string elementName)
        {
            try
            {
                await webView.ExecuteScriptAsync($@"var element = document.getElementById(""{elementName}"");
        if (element) {{
            element.scrollTop = {scrollPosition};
        }} else {{
            alert('Error: Element {elementName} not found.');
        }}");
            }
            catch { }
        }
